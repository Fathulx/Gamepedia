...
if (!$game) {
    $_SESSION['flash'] = ['type' => 'error', 'message' => 'Game tidak ditemukan.'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            $_SESSION['flash'] = ['type' => 'success', 'message' => 'Game berhasil diperbarui.'];
            header('Location: index.php');
            exit;
...
<title>Edit Game | Dashboard Admin</title>
...
<main class="max-w-4xl mx-auto px-5 py-8">
    ...
    <div class="bg-white border border-stone-200 rounded-xl shadow-sm overflow-hidden">
        <div class="border-b border-stone-200 px-6 py-4 bg-stone-50/80 flex items-center gap-3">
            ...
            <span class="text-sm font-medium text-stone-700">Formulir Edit Game</span>
            <span class="ml-auto text-xs text-stone-400">ID: #<?= $id ?></span>
        </div>
        ...
        <form method="POST" class="p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="platform" value="<?= htmlspecialchars($input['platform']) ?>" required
                           class="w-full px-4 py-2.5 border border-stone-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-stone-800 focus:border-transparent transition">
                </div>
